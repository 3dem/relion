...
AutoPickerHip::AutoPickerHip(AutoPickerMpi *basePicker, const char * timing_fnm) :
	basePckr(basePicker),
	allocator(new HipCustomAllocator(0, 1)),
	micTransformer(0, allocator),
	hipTransformer1(0, allocator),
		...
		size_t x, size_t y, size_t mic_size, size_t workSize)
{
	...
	hipLaunchKernelGGL(hip_kernel_convol_B, dim3(Bsize), dim3(BLOCK_SIZE), 0, 0,
												  ~d_Fmic,
												  ~d_Fmsk,
												  ~d_Fcov,
												  d_Fmic.getSize());
	...
	hipLaunchKernelGGL(HIP_KERNEL_NAME(hip_kernel_multi<XFLOAT>), dim3(Bsize), dim3(BLOCK_SIZE), 0, 0,
	...
	hipLaunchKernelGGL(hip_kernel_convol_A, dim3(Bsize), dim3(BLOCK_SIZE), 0, 0,  	  ~d_Fmsk,
													  ~d_Fmic2,
													  ~d_Fcov,
													  d_Fmsk.getSize());
	LAUNCH_HANDLE_ERROR(hipGetLastError());
	...
	Bsize = ( (int) ceilf(( float)d_Mstddev.getSize()/(float)BLOCK_SIZE));
		hipLaunchKernelGGL(hip_kernel_finalizeMstddev, dim3(Bsize), dim3(BLOCK_SIZE), 0, 0,
		~d_Mstddev,
		~hipTransformer2.reals,
		normfft,
		d_Mstddev.getSize());
...
void AutoPickerHip::autoPickOneMicrograph(FileName &fn_mic, long int imic)
{
	...
	else
	{
		...
		hipLaunchKernelGGL(HIP_KERNEL_NAME(HipKernels::hip_kernel_multi<XFLOAT>), dim3(FMultiBsize), dim3(BLOCK_SIZE), 0, 0,
		...
		hipLaunchKernelGGL(hip_kernel_square, dim3(FMultiBsize), dim3(BLOCK_SIZE), 0, 0,
	...
	if(basePckr->do_ctf)
	{
		hipLaunchKernelGGL(hip_kernel_rotateAndCtf, dim3(blocks), dim3(BLOCK_SIZE), 0, 0,
	...
	{
		hipLaunchKernelGGL(hip_kernel_rotateOnly, dim3(blocks), dim3(BLOCK_SIZE), 0, 0,
													  ~hipTransformer1.fouriers,
													  0,
													  projKernel,
													  0
	...
	basePckr->timer.tic(basePckr->TIMING_B5);
			...
			for (int psiIter = 0; psiIter < hipTransformer1.batchIters; psiIter++) // psi-batches for possible memory-limits
			{
				...
				hipLaunchKernelGGL(hip_kernel_batch_convol_A, dim3(blocks2), dim3(BLOCK_SIZE), 0, 0,
					~hipTransformer1.fouriers,
					~d_Fmic,
					FauxStride);
				LAUNCH_HANDLE_ERROR(hipGetLastError());
				...
				hipLaunchKernelGGL(hip_kernel_probRatio, dim3(PR_blocks), dim3(PROBRATIO_BLOCK_SIZE), 0, 0,
						~d_Mccf_best,
						~d_Mpsi_best,
