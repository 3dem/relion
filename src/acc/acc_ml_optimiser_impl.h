		...
		)
{
	...
	{
		int halfset = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_NR_SIGN);
	...
	XX(my_prior)      = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_XOFF_PRIOR);
	YY(my_prior)      = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_YOFF_PRIOR);
	...
	if (baseMLO->mymodel.nr_bodies > 1)
	{
		...
		Euler_angles2matrix(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_ROT),
	...
	if (baseMLO->mymodel.nr_bodies > 1 )
	{
		...
		RFLOAT prior_rot  = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_rot);
		RFLOAT prior_tilt = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_tilt);
		RFLOAT prior_psi =  DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_psi);
	...
	else if (baseMLO->mymodel.orientational_prior_mode != NOPRIOR && !(baseMLO->do_skip_align ||baseMLO-> do_skip_rotate))
	{
		...
		RFLOAT prior_rot = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_ROT_PRIOR);
		RFLOAT prior_tilt = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_TILT_PRIOR);
		RFLOAT prior_psi = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_PSI_PRIOR);
		RFLOAT prior_psi_flip_ratio = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_PSI_PRIOR_FLIP_RATIO);
	...
	else
	{
		...
		else
		{
			...
			if (baseMLO->has_converged && baseMLO->do_use_reconstruct_images)
			{
				...
				{
					DIRECT_A2D_ELEM(rec_img(), i, j) = DIRECT_A3D_ELEM(baseMLO->exp_imagedata, baseMLO->exp_nr_images + op.metadata_offset, i, j);
				}
				rec_img().setXmippOrigin();
			}
	...
	op.Fimg.initZeros(current_size_z, current_size_y, current_size_x);
	...
	if(baseMLO->has_converged && baseMLO->do_use_reconstruct_images)
	{
		...
		AccUtilities::normalizeAndTransformImage<MlClass>(d_rec_img,		// input  acc-side  Array
														  op.Fimg,			// output host-side MultidimArray
														  accMLO,
														  current_size_x,
														  current_size_y,
														  current_size_z);
	...
	baseMLO->mydata.obsModel.demodulatePhase(optics_group, op.Fimg);

	CTOC(accMLO->timer,"selfApplyBeamTilt");

	op.Fimg_nomask = op.Fimg;
	...
	if (baseMLO->mymodel.current_size < baseMLO->mymodel.ori_size)
	{
		...
		op.power_img.resize(baseMLO->mymodel.ori_size/2 + 1);

		for (int i = 0; i<(spectrumAndXi2.getSize()-1); i ++)
			op.power_img.data[i] = spectrumAndXi2[i];
		op.highres_Xi2_img = spectrumAndXi2[spectrumAndXi2.getSize()-1];
	}
	else
	{
		op.highres_Xi2_img = 0.;
	}
	CTOC(accMLO->timer,"powerClass");

	op.Fctf.resize(op.Fimg);
	...
	if (baseMLO->do_ctf_correction)
	{
		if (accMLO->dataIs3D)
		{
			...
			Ictf().setXmippOrigin();
			FOR_ALL_ELEMENTS_IN_FFTW_TRANSFORM(op.Fctf)
			{
				// Use negative kp,ip and jp indices, because the origin in the ctf_img lies half a pixel to the right of the actual center....
				DIRECT_A3D_ELEM(op.Fctf, k, i, j) = A3D_ELEM(Ictf(), -kp, -ip, -jp);
			}
		...
		else
		{
			...
			RFLOAT kV, Cs, Q0;
			baseMLO->mydata.obsModel.opticsMdt.getValue(EMDL_CTF_VOLTAGE, kV, optics_group);
			baseMLO->mydata.obsModel.opticsMdt.getValue(EMDL_CTF_CS, Cs, optics_group);
			baseMLO->mydata.obsModel.opticsMdt.getValue(EMDL_CTF_Q0, Q0, optics_group);

			// Get parameters that change per-particle from the exp_metadata
			CTF ctf;
			ctf.setValues(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, METADATA_CTF_DEFOCUS_U),
			...
			ctf.getFftwImage(op.Fctf, baseMLO->mymodel.ori_size, baseMLO->mymodel.ori_size, baseMLO->mymodel.pixel_size,
	...
	if (baseMLO->mymodel.nr_bodies > 1)
	{
		...
		for (int obody = 0; obody < baseMLO->mymodel.nr_bodies; obody++)
		{
			if (obody != ibody) // Only subtract if other body is not this body....
			{
				...
				Euler_angles2matrix(DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, ocol_rot),
									DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, ocol_tilt),
									DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, ocol_psi), Aresi, false);
		...
		if (baseMLO->do_ctf_correction)
		{
			...
			{
				DIRECT_MULTIDIM_ELEM(Fsum_obody, n) *= DIRECT_MULTIDIM_ELEM(op.Fctf, n);
			}
		}
		...
		Faux = op.Fimg;
		shiftImageInFourierTransform(Faux, op.Fimg, (RFLOAT)baseMLO->mymodel.ori_size,
				XX(my_refined_ibody_offset), YY(my_refined_ibody_offset), ZZ(my_refined_ibody_offset));
		Faux = op.Fimg_nomask;
		shiftImageInFourierTransform(Faux, op.Fimg_nomask, (RFLOAT)baseMLO->mymodel.ori_size,
				XX(my_refined_ibody_offset), YY(my_refined_ibody_offset), ZZ(my_refined_ibody_offset));
	 	...
	 	int ibody = 0)
{
	...
	baseMLO->precalculateShiftedImagesCtfsAndInvSigma2s(false, op.part_id, sp.current_image_size, sp.current_oversampling, op.metadata_offset, // inserted SHWS 12112015
	...
	windowFourierTransform(op.Fimg, Fimg, sp.current_image_size); //TODO PO isen't this already done in getFourierTransformsAndCtfs?
	...
	for (unsigned long i = 0; i < image_size; i ++)
	{
		...
		if (baseMLO->do_ctf_correction && baseMLO->refs_are_ctf_corrected)
		{
			...
			if (op.local_Fctf.data[i]!=0)
				pixel_correction /= op.local_Fctf.data[i];
		}
	...
	buildCorrImage(baseMLO,op,corr_img,group_id);
	corr_img.cpToDevice();

	deviceInitValue<XFLOAT>(allWeights, (XFLOAT) (op.highres_Xi2_img / 2.));
	allWeights_pos = 0;
											...
											int ibody)
{
	...
	// Ready the "prior-containers" for all classes
	...
	else
	{
		...
		// loop through making translational priors for all classes - then copy all at once - then loop through kernel calls ( TODO: group kernel calls into one big kernel)
		...
		if(exp_ipass==0)
		{
			AccPtr<XFLOAT>  ipartMweight(
					Mweight,
					sp.nr_dir * sp.nr_psi * sp.nr_trans * sp.iclass_min,
			...
			*/
			...
			if (ipart_length > 1)
			{
				...
				if (my_nr_significant_coarse_samples == 0)
				{
					...
					std::cerr << " op.sum_weight= " << op.sum_weight << std::endl;
					std::cerr << " min_diff2= " << op.min_diff2 << std::endl;
		...
		else
		{
			...
			if(baseMLO->adaptive_oversampling!=0)
			{
				...
				if (op.sum_weight==0)
				{
					std::cerr << std::endl;
					std::cerr << " fn_img= " << sp.current_img << std::endl;
					std::cerr << " part_id= " << op.part_id << std::endl;
					...
					CRITICAL(ERRSUMWEIGHTZERO); //"op.sum_weight==0"
				}
						...
						AccPtrBundle &bundleSWS)
{
	...
	// Set those back here
	int group_id = baseMLO->mydata.getGroupId(op.part_id);
	DIRECT_MULTIDIM_ELEM(op.local_Minvsigma2, 0) = 1. / (baseMLO->sigma2_fudge * DIRECT_A1D_ELEM(baseMLO->mymodel.sigma2_noise[group_id], 0));
	...
	unsigned long image_size = op.Fimg.nzyxdim;
	...
	std::vector<long int> block_nums(nr_fake_classes);
	...
	for (long int exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++)
	{
		...
		int block_num = block_nums[fake_class];
	...
	for (long int iclass = sp.iclass_min; iclass <= sp.iclass_max; iclass++)
	{
		...
		{
			iorient= FinePassWeights.rot_id[FPCMasks[iclass].jobOrigin[n-partial_pos]+FPCMasks[iclass].firstPos];
	...
	if(baseMLO->adaptive_oversampling!=0)
		op.max_index.fineIndexToFineIndices(sp); // set partial indices corresponding to the found max_index, to be used below
	else
		op.max_index.coarseIndexToCoarseIndices(sp);

	baseMLO->sampling.getTranslations(op.max_index.itrans, baseMLO->adaptive_oversampling,
			oversampled_translations_x, oversampled_translations_y, oversampled_translations_z,
			(baseMLO->do_helical_refine) && (! baseMLO->ignore_helical_symmetry), baseMLO->helical_rise_initial / baseMLO->mymodel.pixel_size, baseMLO->helical_twist_initial);
	...
	if(baseMLO->do_skip_align || baseMLO->do_skip_rotate)
		   ...
		   baseMLO->sampling.getOrientations(op.max_index.idir, op.max_index.ipsi, baseMLO->adaptive_oversampling, oversampled_rot, oversampled_tilt, oversampled_psi,
				op.pointer_dir_nonzeroprior, op.directions_prior, op.pointer_psi_nonzeroprior, op.psi_prior);

	baseMLO->sampling.getOrientations(op.max_index.idir, op.max_index.ipsi, baseMLO->adaptive_oversampling, oversampled_rot, oversampled_tilt, oversampled_psi,
			op.pointer_dir_nonzeroprior, op.directions_prior, op.pointer_psi_nonzeroprior, op.psi_prior);

	RFLOAT rot = oversampled_rot[op.max_index.ioverrot];
	RFLOAT tilt = oversampled_tilt[op.max_index.ioverrot];
	RFLOAT psi = oversampled_psi[op.max_index.ioverrot];
	...
	RFLOAT old_rot = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_rot);
	DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_rot) = rot;
	RFLOAT old_tilt = DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_tilt);
	DIRECT_A2D_ELEM(baseMLO->exp_metadata, op.metadata_offset, icol_tilt) = tilt;
	...
	if (accMLO->dataIs3D)
	{
		ZZ(shifts) = oversampled_translations_z[op.max_index.iovertrans];
	}

	// Use oldpsi-angle to rotate back the XX(exp_old_offset) + oversampled_translations_x[iover_trans] and
	if ( (baseMLO->do_helical_refine) && (! baseMLO->ignore_helical_symmetry) )
	...
	windowFourierTransform(op.Fimg, Fimg, sp.current_image_size); //TODO PO isen't this already done in getFourierTransformsAndCtfs?
	windowFourierTransform(op.Fimg_nomask, Fimg_nonmask, sp.current_image_size);
	...
	{
		for (unsigned long i = 0; i < image_size; i++)
			ctfs[i] = (XFLOAT) op.local_Fctf.data[i] * part_scale;
	}
	...
	for (unsigned long exp_iclass = sp.iclass_min; exp_iclass <= sp.iclass_max; exp_iclass++)
	{
		...
		for (long int j = 0; j < image_size; j++)
		{
			...
			{
				DIRECT_A1D_ELEM(exp_wsum_scale_correction_AA, ires) += wdiff2s_AA[AAXA_pos+j];
				DIRECT_A1D_ELEM(exp_wsum_scale_correction_XA, ires) += wdiff2s_XA[AAXA_pos+j];
			}
		}
		AAXA_pos += image_size;
	} // end loop iclass
	for (unsigned long j = 0; j < image_size; j++)
	{
		...
		if (ires > -1)
		{
			...
			exp_wsum_norm_correction += (RFLOAT) wdiff2s_sum[j]; //TODO could be gpu-reduced
		}
	}
	...
	{
		DIRECT_A1D_ELEM(thr_wsum_sigma2_noise[group_id], ires) += DIRECT_A1D_ELEM(op.power_img, ires);
		// Also extend the weighted sum of the norm_correction
		exp_wsum_norm_correction += DIRECT_A1D_ELEM(op.power_img, ires);
	}
...
// ----------------------------------------------------------------------------
template <class MlClass>
void accDoExpectationOneParticle(MlClass *myInstance, unsigned long my_part_id, int thread_id, AccPtrFactory ptrFactory)
{
	SamplingParameters sp;
	...
	OptimisationParamters op(my_part_id);
	...
	if (baseMLO->do_generate_seeds)
	{
				...
				(!baseMLO->do_firstiter_cc && baseMLO->iter == 1))
		{
			...
			long int idx = my_part_id - baseMLO->exp_my_first_part_id;
    ...
    for (int ibody = 0; ibody < baseMLO->mymodel.nr_bodies; ibody++)
    {
    	...
    	op.metadata_offset = my_part_id - baseMLO->exp_my_first_part_id;
#ifdef TIMING
// Only time one thread
if (thread_id == 0)
baseMLO->timer.toc(baseMLO->TIMING_ESP_DIFF2_A);
		...
		getFourierTransformsAndCtfs<MlClass>(my_part_id, op, sp, baseMLO, myInstance, ptrFactory, ibody);
		...
		if (baseMLO->do_skip_align)
		{
			sp.itrans_min = sp.itrans_max = sp.idir_min = sp.idir_max = sp.ipsi_min = sp.ipsi_max =
					my_part_id - baseMLO->exp_my_first_part_id;
		}
		else
		{
			sp.itrans_min = 0;
		...
		IndexedDataArray CoarsePassWeights = ptrFactory;
		IndexedDataArray FinePassWeights = ptrFactory;
		...
		std::vector <IndexedDataArrayMask > FinePassClassMasks(std::vector <IndexedDataArrayMask >(baseMLO->mymodel.nr_classes, ptrFactory));
