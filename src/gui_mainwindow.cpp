...
void GuiMainWindow::fillToAndFromJobLists()
{
	...
	if (current_job >= 0)
	{
		...
		for (long int inode = 0; inode < (pipeline.processList[current_job]).inputNodeList.size(); inode++)
		{
			...
			if (pipeline.nodeList[mynode].type != NODE_MOVIES_CPIPE) // no display for movie rootname
			{
...
void GuiMainWindow::cb_display_io_node_i()
{
	...
	if (pipeline.nodeList[mynode].type == NODE_COORDS_CPIPE)
	{
	...
	else if (pipeline.nodeList[mynode].type == NODE_LOGFILE_CPIPE)
	{
	...
	else if (pipeline.nodeList[mynode].type == OUTNODE_POLISH_PARAMS)
	{
		command = "cat " + pipeline.nodeList[mynode].name;
	}
	else if (pipeline.nodeList[mynode].type != OUTNODE_POST)
	{
