...
//	HANDLE_ERROR(cudaMemcpy( ~out, ~in, in.size * sizeof(T), cudaMemcpyDeviceToDevice));
...
//	thrust::sort(dv.begin(), dv.end() );
	...
	HANDLE_ERROR(cudaMemcpy( h_vec, ~in, in.size * sizeof(T), cudaMemcpyDeviceToHost));
	thrust::sort(h_vec, h_vec + in.size);
	HANDLE_ERROR(cudaMemcpy( ~out, h_vec, in.size * sizeof(T), cudaMemcpyHostToDevice));
	delete [] h_vec;
}
