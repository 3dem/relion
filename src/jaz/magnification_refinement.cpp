...
void MagnificationRefinement::updateScaleFreq(
        const Image<Complex> &prediction, const Image<Complex> &observation,
        CTF &ctf, RFLOAT angpix, Volume<Equation2x2> &eqs)
{
...
void MagnificationRefinement::updateScaleReal(
        ...
        Volume<Equation2x2> &eqs)
{
    ...
    for (long x = 0; x < ww; x++)
    {
        RFLOAT vx = DIRECT_A2D_ELEM(realPred.data, y, x);
        RFLOAT vy = DIRECT_A2D_ELEM(realObs.data, y, x);
...
void MagnificationRefinement::solvePerPixel(
...
void MagnificationRefinement::solveLinearlyFreq(
        const Volume<Equation2x2> &eqs,
        const Image<RFLOAT>& snr,
        Image<RFLOAT> &mat,
        Image<RFLOAT> &vx, Image<RFLOAT> &vy)
{
    ...
    for (long xi = 1; xi < w-1; xi++)
    {
        ...
        RFLOAT wgh = DIRECT_A2D_ELEM(snr.data, yi, xi);
...
void MagnificationRefinement::readEQs(std::string path, Volume<Equation2x2> &eqs)
{
...
void MagnificationRefinement::writeEQs(const Volume<Equation2x2> &eqs, std::string path)
{
    const long w = eqs.dimx;
    const long h = eqs.dimy;
...
void MagnificationRefinement::updatePowSpec(const Image<Complex> &prediction,
                              const Image<Complex> &observation,
                              CTF &ctf, RFLOAT angpix,
