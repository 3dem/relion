...
Whichever program you use, 'Motion Refinement' is highly recommended to get the most of your dataset.");
	...
	joboptions["fn_defect"] = JobOption("Defect file:", "", "*", ".", "Location of the MOTIONCOR2-style ASCII text file  that describes the defect pixels on the detector (using the -DefectFile option). Leave empty if you don't have any defects, or don't want to correct for defects on your detector.");
		...
		std::string &final_command, bool do_makedir, int job_counter, std::string &error_message)
{
	...
	if ( (!is_continue) && (joboptions["do_helix"].getBoolean()) )
	{
		...
		if ( (joboptions["dont_skip_align"].getBoolean()) && (!joboptions["do_local_ang_searches"].getBoolean()) )
		{
			...
			val = (val > 90.) ? (90.) : (val);
			command += " --sigma_psi " + floatToString(val / 3.);
			val = textToFloat(joboptions["range_rot"].getString());
			val = (val < 0.) ? (0.) : (val);
			val = (val > 90.) ? (90.) : (val);
