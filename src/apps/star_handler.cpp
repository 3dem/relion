...
class star_handler_parameters
{
	...
	void select_by_str()
	{
        ...
        // Remove optics groups that are no longer in use, and renumber in MDout
        obsModel.removeUnusedOpticsGroups(MDout);
	...
	void combine()
	{
        ...
        MetaDataTable MDout;
        combineStarfiles(fns_in, MDout, obsModel, fn_check, do_ignore_optics, tablename_in, 1);

		write_check_ignore_optics(MDout, fn_out, MDout.getName());
		std::cout << " Written: " << fn_out << std::endl;
	}
